...
| **Working Memory (SRAM)** | `ml/classical/nlms.py` | Algorithmic state analysis | N/A | N/A | 32-tap FIR filter vector | State vector size: $N \times 4\text{ bytes} = 128\text{ bytes}$ | **ESTIMATED (ALGORITHMIC ANALYSIS)** | "Projected working-memory requirement: < 1 KB (128 B filter state), well within the nRF52840's available 256 KB RAM budget." |
