...
| **Mechanical** | **Hybrid Rigid-Fabric (Polar H10 Style)** | The MCU/AFE is printed on a cheap Rigid FR4 PCB (The Brain). This brain snaps into a washable maternity belt (The Arms) which houses the Graphene electrodes. Replaces expensive full-FPC fabrication. |
...
*   6–8 abdominal biopotential pickup positions distributed across upper/lower and left/right quadrants, plus the required reference/ground/RLD arrangement defined by the AFE design.
*   Place two PVDF sensors at spatially separated locations so their signals can be compared.
*   Put the IMU near the belt's mechanical center.

> [!IMPORTANT]
> Exact electrode geometry is a prototype hypothesis, not a clinical standard. Validate placement using phantom/signal-injection tests before any human study.
